      ...
      for ($i=1; $i<=$n; $i++)
      {
        ...
        if ((!empty($dataNama)) && (!empty($dataJml)))
        {
          $query = "INSERT INTO alat (id_alat, jenis, id_lab, nama_alat, ukuran, merk, jumlah, ket)
                    VALUES ('', 'alat','$id_lab', '$dataNama', '$dataUkuran','$dataMerk','$dataJml','$dataKet') ";
          $result = $db->query($query);
          if (!$result)
          {
            die ("Could not query the database: <br />". $db->error);
            echo '<script language="javascript">document.location="daftar_alat.php";</script>';
          }
        }else{
           echo '<script language="javascript">alert("Ada yang salah! Mohon hubungi pihak yang bersangkutan"); document.location="daftar_alat.php";</script>';
        }
      }
      echo '<script language="javascript">alert("Proses pemasukan data alat sukses!"); document.location="daftar_alat.php";</script>';
    }
?>
                      ...
                      <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                          <?php
                            ...
                            {
                              echo '<tr>
                                      <td><center><input type="text" name="nama_alat'.$i.'" required></center></td>
